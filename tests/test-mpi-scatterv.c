...
static MunitResult test_scatterv(const MunitParameter params[], void* data)
{
    ...
    int* send_sizes = malloc(size*sizeof(int));
    ...
    ret = MPI_Scatterv(send_buf, send_sizes, offsets, MPI_BYTE,
                       recv_buf, 8096+rank*64, MPI_BYTE, 0, context->mpi_comm);
    munit_assert_int(ret, ==, 0);
    ...
    // scatterv from another root
    memset(recv_buf, 0, 8096+rank*64);
    int root = size/2;
    ret = MPI_Scatterv(send_buf, send_sizes, offsets, MPI_BYTE,
                       recv_buf, 8096+rank*64, MPI_BYTE, root, context->mpi_comm);
    munit_assert_int(ret, ==, 0);
...
static MunitTest test_suite_tests[] = {
    { (char*) "/scatterv", test_scatterv, test_context_setup, test_context_tear_down, MUNIT_TEST_OPTION_NONE, NULL },
