                                           ...
                                           void*                data)
{
    ...
    if (context->rank == 0) { // sender
        ...
        int i;
        for (i = mona_msg_get_expected_header_size(mona); i < (int)msg_len;
             i++) {
            buf[i] = i % 32;
        }
        ...
        ret = mona_msg_send_expected(mona, buf, msg_len, plugin_data,
                                     context->other_addr, 0, 0);
        munit_assert_int(ret, ==, NA_SUCCESS);
        ...
        ret = mona_msg_irecv_expected(mona, buf, msg_len, plugin_data,
