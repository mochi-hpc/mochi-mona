...
static MunitResult test_gatherv(const MunitParameter params[], void* data)
{
    ...
    // gatherv to rank 0
    ret = MPI_Gatherv(send_buf, 8096 + rank * 64, MPI_BYTE, recv_buf,
                      recv_sizes, offsets, MPI_BYTE, 0, context->mpi_comm);
    munit_assert_int(ret, ==, 0);
    ...
    int root = size / 2;
    ret      = MPI_Gatherv(send_buf, 8096 + rank * 64, MPI_BYTE, recv_buf,
                           recv_sizes, offsets, MPI_BYTE, root, context->mpi_comm);
    munit_assert_int(ret, ==, 0);
