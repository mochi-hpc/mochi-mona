...
static MunitResult test_send_recv_unexpected(const MunitParameter params[], void* data)
{
    (void)params;
    ...
    if(context->rank == 0) { // sender
        ...
        ret = mona_msg_send_unexpected(
                mona, buf, msg_len, plugin_data, context->other_addr, 0, 1234);
        munit_assert_int(ret, ==, NA_SUCCESS);
    } else { // receiver
        na_tag_t  tag         = 0;
...
static MunitTest test_suite_tests[] = {
    { (char*) "/unexpected", test_send_recv_unexpected, test_context_setup, test_context_tear_down, MUNIT_TEST_OPTION_NONE, NULL },
