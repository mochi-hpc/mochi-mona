...
static MunitResult test_allgatherv(const MunitParameter params[], void* data)
{
    (void)params;
    ...
    size_t* recv_sizes = malloc(size*sizeof(size_t));
    ...
    ret = mona_comm_allgatherv(context->comm, send_buf, 8096+rank*64,
                               recv_buf, recv_sizes, offsets, 1234);
    munit_assert_int(ret, ==, NA_SUCCESS);
...
static MunitTest test_suite_tests[] = {
    { (char*) "/allgatherv", test_allgatherv, test_context_setup, test_context_tear_down, MUNIT_TEST_OPTION_NONE, NULL },
