...
static MunitResult test_isend_irecv_multi(const MunitParameter params[],
                                          void*                data)
{
    (void)params;
    ...
    if (context->rank == 0) { // sender
        ...
        ret = mona_send(mona, buf + msg_len, msg_len, context->other_addr, 0,
                        2);
        munit_assert_int(ret, ==, NA_SUCCESS);

        ret = mona_send(mona, buf + msg_len * 2, msg_len, context->other_addr,
                        0, 3);
        munit_assert_int(ret, ==, NA_SUCCESS);

        ret = mona_send(mona, buf + msg_len * 3, msg_len, context->other_addr,
                        0, 4);
        munit_assert_int(ret, ==, NA_SUCCESS);
        ...
        ret = mona_irecv(mona, buf, msg_len, context->other_addr, 4, recv_size,
                         req);
        munit_assert_int(ret, ==, NA_SUCCESS);

        ret = mona_irecv(mona, buf + msg_len * 2, msg_len, context->other_addr,
                         2, recv_size + 1, req + 1);
        munit_assert_int(ret, ==, NA_SUCCESS);

        ret = mona_irecv(mona, buf + msg_len * 3, msg_len, context->other_addr,
                         3, recv_size + 2, req + 2);
        munit_assert_int(ret, ==, NA_SUCCESS);

        ret = mona_irecv(mona, buf + msg_len, msg_len, context->other_addr, 1,
                         recv_size + 3, req + 3);
        munit_assert_int(ret, ==, NA_SUCCESS);
...
static MunitTest test_suite_tests[]
    = {{(char*)"/hl", test_isend_irecv_multi, test_context_setup,
        test_context_tear_down, MUNIT_TEST_OPTION_NONE, NULL},
       {NULL, NULL, NULL, NULL, MUNIT_TEST_OPTION_NONE, NULL}};

static const MunitSuite test_suite
    = {(char*)"/mona/isend-irecv-multi", test_suite_tests, NULL, 1,
