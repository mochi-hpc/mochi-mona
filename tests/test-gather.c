...
static MunitResult test_gather(const MunitParameter params[], void* data)
{
    ...
    // gather to rank 0
    ret = mona_comm_gather(context->comm, send_buf, 8096, recv_buf, 0, 1234);
    munit_assert_int(ret, ==, NA_SUCCESS);
    ...
    int root = size / 2;
    ret = mona_comm_gather(context->comm, send_buf, 8096, recv_buf, root, 1234);
    munit_assert_int(ret, ==, NA_SUCCESS);
...
static MunitTest test_suite_tests[]
    = {{(char*)"/gather", test_gather, test_context_setup,
