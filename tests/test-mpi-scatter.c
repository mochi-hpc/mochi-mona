...
static MunitResult test_scatter(const MunitParameter params[], void* data)
{
    ...
    if (rank == 0) {
        for (i = 0; i < size * 8096; i++) { send_buf[i] = 'A' + (i / 8096); }
    }

    // scatter from rank 0
    ret = MPI_Scatter(send_buf, 8096, MPI_BYTE, recv_buf, 8096, MPI_BYTE, 0,
                      context->mpi_comm);
    munit_assert_int(ret, ==, 0);
    for (i = 0; i < 8096; i++) {
    ...
    ret = MPI_Scatter(send_buf, 8096, MPI_BYTE, recv_buf, 8096, MPI_BYTE, root,
                      context->mpi_comm);
    munit_assert_int(ret, ==, 0);
    for (i = 0; i < 8096; i++) {
        munit_assert_int(recv_buf[i], ==, 'A' + rank);
...
static MunitTest test_suite_tests[]
    = {{(char*)"/scatter", test_scatter, test_context_setup,
