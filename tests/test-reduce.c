...
static MunitResult test_reduce(const MunitParameter params[], void* data)
{
    (void)params;
    ...
    // reduce to rank 0
    ret = mona_comm_reduce(context->comm, &send_val, &recv_val,
                           sizeof(send_val), 1, mona_op_sum_u64, NULL, 0, 1234);
    munit_assert_int(ret, ==, NA_SUCCESS);
    if (rank == 0) munit_assert_int(recv_val, ==, size * (size - 1));

    // reduce to another root
    int root = size / 2;
    ret      = mona_comm_reduce(context->comm, &send_val, &recv_val,
                                sizeof(send_val), 1, mona_op_sum_u64, NULL, root,
                                1234);
    munit_assert_int(ret, ==, NA_SUCCESS);
    if (rank == root) munit_assert_int(recv_val, ==, size * (size - 1));

    return MUNIT_OK;
}

static MunitTest test_suite_tests[]
    = {{(char*)"/reduce", test_reduce, test_context_setup,
