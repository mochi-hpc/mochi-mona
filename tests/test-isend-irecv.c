...
static MunitResult test_isend_irecv(const MunitParameter params[], void* data)
{
    ...
    if (context->rank == 0) { // sender
        ...
        ret = mona_isend(mona, buf, msg_len, context->other_addr, 0, 1234,
                         &req);
        munit_assert_int(ret, ==, NA_SUCCESS);
        ...
        size_t recv_size;
        ret = mona_irecv(mona, buf, 64, context->other_addr, 1234, &recv_size,
                         &req);
        munit_assert_int(ret, ==, NA_SUCCESS);
        ...
        mona_request_t req;
        size_t         recv_size;
        ret = mona_irecv(mona, buf, msg_len, context->other_addr, 1234,
                         &recv_size, &req);
        munit_assert_int(ret, ==, NA_SUCCESS);
...
static MunitTest test_suite_tests[]
    = {{(char*)"/hl", test_isend_irecv, test_context_setup,
        test_context_tear_down, MUNIT_TEST_OPTION_NONE, NULL},
       {NULL, NULL, NULL, NULL, MUNIT_TEST_OPTION_NONE, NULL}};

static const MunitSuite test_suite
    = {(char*)"/mona/isend-irecv", test_suite_tests, NULL, 1,
