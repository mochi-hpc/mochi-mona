...
static MunitResult test_send_recv(const MunitParameter params[], void* data)
{
    ...
    comm_addrs[0]
        = context->rank == 0 ? context->self_addr : context->other_addr;
    comm_addrs[1]
        = context->rank == 0 ? context->other_addr : context->self_addr;
    ...
    MPI_Status status;
    mpi_ret
        = MPI_Sendrecv(buf + context->rank * msg_len / 2, // sendbuf
                       msg_len / 2,                       // sendcount
                       MPI_BYTE,                          // sendtype
...
static MunitResult test_isend_irecv(const MunitParameter params[], void* data)
{
    ...
    if (context->rank == 0) { // sender
        ...
        mpi_ret
            = MPI_Isend(buf, msg_len, MPI_BYTE, 1, 1234, mpi_comm, &send_req);
        munit_assert_int(mpi_ret, ==, 0);
        printf("send_req = %d\n", send_req);
        ...
        MPI_Request recv_req = MPI_REQUEST_NULL;
        mpi_ret
            = MPI_Irecv(buf, msg_len, MPI_BYTE, 0, 1234, mpi_comm, &recv_req);
        munit_assert_int(mpi_ret, ==, 0);
...
static MunitTest test_suite_tests[]
    = {{(char*)"/send-recv", test_send_recv, test_context_setup,
        test_context_tear_down, MUNIT_TEST_OPTION_NONE, NULL},
       {(char*)"/isend-irecv", test_isend_irecv, test_context_setup,
        test_context_tear_down, MUNIT_TEST_OPTION_NONE, NULL},
