...
static MunitResult test_put_get(const MunitParameter params[], void* data)
{
    ...
    ret = mona_mem_handle_create(mona, bulk_buf, 1024, NA_MEM_READWRITE,
                                 &mem_handle);
    munit_assert_int(ret, ==, NA_SUCCESS);
    ...
    size_t mem_handle_size
        = mona_mem_handle_get_serialize_size(mona, mem_handle);

    if (context->rank == 0) { // sender

        // set the buffer content
        int i;
        for (i = 0; i < 1024; i++) { bulk_buf[i] = i % 32; }
        ...
        ret = mona_mem_handle_serialize(
            mona, msg_buf + mona_msg_get_unexpected_header_size(mona),
            mem_handle_size, mem_handle);
        munit_assert_int(ret, ==, NA_SUCCESS);

        // send unexpected message
        ret = mona_msg_send_unexpected(mona, msg_buf, msg_len, plugin_data,
                                       context->other_addr, 0, 1234);
        munit_assert_int(ret, ==, NA_SUCCESS);

        // receive unexpected message
        ret = mona_msg_recv_unexpected(mona, msg_buf, msg_len, plugin_data,
                                       NULL, NULL, NULL);
        munit_assert_int(ret, ==, NA_SUCCESS);
        ...
        ret                                 = mona_mem_handle_deserialize(
            mona, &remote_mem_handle,
            msg_buf + mona_msg_get_unexpected_header_size(mona),
            mem_handle_size);
        munit_assert_int(ret, ==, NA_SUCCESS);

        // issue GET
        ret = mona_get(mona, mem_handle, 0, remote_mem_handle, 0, 1024,
                       context->other_addr, 0);
        munit_assert_int(ret, ==, NA_SUCCESS);
        ...
        // modify buffer
        for (i = 0; i < 1024; i++) { bulk_buf[i] = (i + 1) % 32; }

        // issue PUT
        ret = mona_put(mona, mem_handle, 0, remote_mem_handle, 0, 1024,
                       context->other_addr, 0);
        munit_assert_int(ret, ==, NA_SUCCESS);
...
static MunitTest test_suite_tests[]
    = {{(char*)"/put-get", test_put_get, test_context_setup,
