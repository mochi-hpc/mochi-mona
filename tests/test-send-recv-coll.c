...
static MunitResult test_send_recv(const MunitParameter params[], void* data)
{
    ...
    if (context->rank == 0) { // sender
        ...
        size_t recv_size;
        ret = mona_comm_recv(comm, buf, msg_len, 0, 1234, &recv_size, NULL,
                             NULL);
        munit_assert_int(ret, ==, NA_SUCCESS);
    ...
    ret = mona_comm_sendrecv(comm, buf + context->rank * msg_len / 2,
                             msg_len / 2, (context->rank + 1) % 2, 1234,
                             buf + ((context->rank + 1) % 2) * msg_len / 2,
                             msg_len / 2, (context->rank + 1) % 2, 1234, NULL);
    munit_assert_int(ret, ==, NA_SUCCESS);
...
static MunitTest test_suite_tests[]
    = {{(char*)"/comm", test_send_recv, test_context_setup,
