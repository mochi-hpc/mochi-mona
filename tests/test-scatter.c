...
static void* test_context_setup(const MunitParameter params[], void* user_data)
{
    ...
    MPI_Init(NULL, NULL);
    ABT_init(0, NULL);
    mona_instance_t mona = mona_init("ofi+tcp", false, NULL);
...
static MunitResult test_scatter(const MunitParameter params[], void* data)
{
    (void)params;
    ...
    char* send_buf = calloc(1, 8096*size);
    char* recv_buf = calloc(1, 8096);

    if(rank == 0) {
        for(i = 0; i < size*8096; i++) {
            send_buf[i] = 'A' + (i/8096);
        }
    }

    // scatter from rank 0
    ret = mona_comm_scatter(context->comm, send_buf, 8096, recv_buf, 0, 1234);
    munit_assert_int(ret, ==, NA_SUCCESS);
    for(i = 0; i < 8096; i++) {
        munit_assert_int(recv_buf[i], ==, 'A' + rank);
    }

    // scatter from another root
    memset(recv_buf, 0, 8096);
    memset(send_buf, 0, 8096*size);
    int root = size/2;
    if(rank == root) {
        for(i = 0; i < size*8096; i++) {
            send_buf[i] = 'A' + (i/8096);
    ...
    ret = mona_comm_scatter(context->comm, send_buf, 8096, recv_buf, root, 1234);
    munit_assert_int(ret, ==, NA_SUCCESS);
    for(i = 0; i < 8096; i++) {
        munit_assert_int(recv_buf[i], ==, 'A' + rank);
    }
...
static MunitTest test_suite_tests[] = {
    { (char*) "/scatter", test_scatter, test_context_setup, test_context_tear_down, MUNIT_TEST_OPTION_NONE, NULL },
