...
static void* test_context_setup(const MunitParameter params[], void* user_data)
{
    ...
    ret = NA_Addr_to_string(context->na_class, self_addr_str, &self_addr_size,
                            context->self_addr);
    munit_assert_int(ret, ==, NA_SUCCESS);
    ...
    ret = NA_Addr_lookup(context->na_class, other_addr_str,
                         &(context->other_addr));
    munit_assert_int(ret, ==, NA_SUCCESS);

    context->msg_len
        = MIN(NA_Msg_get_max_expected_size(context->na_class),
              2 * NA_Msg_get_max_unexpected_size(context->na_class));
    context->buf = (char*)NA_Msg_buf_alloc(context->na_class, context->msg_len,
                                           0, &(context->plugin_data));
...
static MunitResult test_send_recv_expected(const MunitParameter params[],
                                           void*                data)
{
    (void)params;
    ...
    if (context->rank == 0) { // sender
        ...
        int i;
        for (i = NA_Msg_get_expected_header_size(context->na_class);
             i < (int)context->msg_len; i++) {
            context->buf[i] = i % 32;
        }
        ret = NA_Msg_init_expected(context->na_class, context->buf,
                                   context->msg_len);
        munit_assert_int(ret, ==, NA_SUCCESS);

        MPI_Barrier(MPI_COMM_WORLD);

        ret = NA_Msg_send_expected(context->na_class, context->na_context,
                                   sender_callback, context, context->buf,
                                   context->msg_len, context->plugin_data,
                                   context->other_addr, 0, 0, context->op_id);
        munit_assert_int(ret, ==, NA_SUCCESS);

    } else { // receiver

        ret = NA_Msg_recv_expected(context->na_class, context->na_context,
                                   receiver_callback, context, context->buf,
                                   context->msg_len, context->plugin_data,
                                   context->other_addr, 0, 0, context->op_id);
        munit_assert_int(ret, ==, NA_SUCCESS);
...
static MunitTest test_suite_tests[]
    = {{(char*)"/expected", test_send_recv_expected, test_context_setup,
