...
static MunitResult test_waitany(const MunitParameter params[], void* data)
{
    ...
    char sendbuf[256];
    for (unsigned i = 0; i < 256; i++) { sendbuf[i] = 'a' + ((rank + i) % 26); }
    ...
    for (int i = 0; i < size; i++) {
        ...
        ret = MPI_Isend(sendbuf, 256, MPI_BYTE, i, 0, context->mpi_comm,
                        sendreqs + i);
        munit_assert_int(ret, ==, 0);
    }
    ...
    for (int i = 0; i < size - 1; i++) {
        if (sendreqs2[i] != MPI_REQUEST_NULL) MPI_Request_free(sendreqs2 + i);
        if (recvreqs2[i] != MPI_REQUEST_NULL) MPI_Request_free(recvreqs2 + i);
    }
...
static MunitTest test_suite_tests[]
    = {{(char*)"/waitany", test_waitany, test_context_setup,
