    ...
    if(rank == root) {
        ...
        for(i = 0; i < comm_size; i++) {
            ...
            na_ret = mona_comm_irecv(
                    comm, (char*)recvbuf + i*size,
                    size, i, tag, NULL, NULL, NULL, reqs + i);
            if(na_ret != NA_SUCCESS)
                goto finish;
        }
        for(i = 0; i < comm_size; i++) {
...
static void igather_thread(void* x)
{
    ...
    na_return_t na_ret = mona_comm_gather(
            args->comm,
            args->sendbuf,
            args->size,
            args->recvbuf,
    ...
    if(rank == root) {
        ...
        for(i = 0; i < comm_size; i++) {
            ...
            if(i == rank) {
                ...
                if(sendbuf != MONA_IN_PLACE) {
                    memcpy((char*)recvbuf + offsets[i], sendbuf,
                            recvsizes[i]);
                }
                continue;
            }
    ...
    if(root == rank) {
        ...
        for(i = 0; i < comm_size; i++) {
            ...
            na_ret = mona_comm_isend(
                    comm, sendaddr, size, i, tag, reqs+i);
            if(na_ret != NA_SUCCESS)
                goto finish;
        }
        for(i = 0; i < comm_size; i++) {
...
static void iscatter_thread(void* x)
{
    ...
    na_return_t na_ret = mona_comm_scatter(
            args->comm,
            args->sendbuf,
            args->size,
            args->recvbuf,
        ...
        na_tag_t tag)
{
    ...
    if(root == rank) {
        ...
        for(i = 0; i < comm_size; i++) {
            ...
            if(i == rank) {
                memcpy(recvbuf, sendaddr, recvsize < sendsizes[i] ? recvsize : sendsizes[i]);
                continue;
            }
            na_ret = mona_comm_isend(
                    comm, sendaddr, sendsizes[i], i, tag, reqs+i);
            if(na_ret != NA_SUCCESS)
                goto finish;
        }
        for(i = 0; i < comm_size; i++) {
        ...
        na_ret = mona_comm_recv(
                comm, recvbuf, recvsize,
                root, tag, NULL, NULL, NULL);
    }
finish:
    free(reqs);
    return na_ret;
    ...
    while(mask < comm_size) {
        ...
        if((mask & relrank) == 0) {
            ...
            if(source < comm_size) {
                ...
                na_ret = mona_comm_recv(
                        comm, tempSrc, typesize*count,
                        source, tag, NULL, NULL, NULL);
                if(na_ret != NA_SUCCESS) {
                    goto finish;
                }
            ...
            na_ret = mona_comm_send(comm, recvbuf, typesize*count, source, tag);
        ...
        na_tag_t tag)
{
    ...
    na_return_t na_ret;
    int root = 0;
    na_ret = mona_comm_reduce(
            comm, sendbuf, recvbuf, typesize, count,
            op, uargs, root, tag);
    if(na_ret != NA_SUCCESS) {
        return na_ret;
    }
    ...
    // bcast
    na_ret = mona_comm_bcast(
            comm, recvbuf, typesize*count, root, tag);
    return na_ret;
}
