...
static void run_mpi_benchmark(options_t* options) {
    ...
    char* recv_buf = malloc(options->msg_size * size);

    // warm up
    MPI_Allgather(send_buf, options->msg_size, MPI_BYTE, recv_buf, options->msg_size, MPI_BYTE, MPI_COMM_WORLD);
    ...
    free(send_buf);
    free(recv_buf);
    if(rank == 0) printf("MPI: %d allgather of %d bytes executed in %lf sec\n",
...
static void run_mona_benchmark(options_t* options) {
    ...
    // warm up
    mona_comm_allgather(comm, send_buf, options->msg_size, recv_buf, 0);
    ...
    mona_finalize(mona);
    if(rank == 0) printf("MoNA: %d allgather of %d bytes executed in %lf sec\n",
            options->iterations, options->msg_size, (t_end-t_start));
}
