...
static void run_mpi_benchmark(options_t* options) {
    ...
    // warm up
    MPI_Alltoall(send_buf, options->msg_size, MPI_BYTE, recv_buf, options->msg_size, MPI_BYTE, MPI_COMM_WORLD);
    ...
    free(send_buf);
    free(recv_buf);
    if(rank == 0) printf("MPI: %d alltoall of %d bytes executed in %lf sec\n",
...
static void run_mona_benchmark(options_t* options) {
    ...
    // warm up
    mona_comm_alltoall(comm, send_buf, options->msg_size, recv_buf, 0);
