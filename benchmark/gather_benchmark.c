...
static void run_mpi_benchmark(options_t* options) {
    ...
    // warm up
    MPI_Gather(send_buf, options->msg_size, MPI_BYTE, recv_buf, options->msg_size, MPI_BYTE, 0, MPI_COMM_WORLD);
    // benchmark
    ...
    free(send_buf);
    free(recv_buf);
    if(rank == 0) printf("MPI: %d gather of %d bytes executed in %lf sec\n",
...
static void run_mona_benchmark(options_t* options) {
    ...
    // warm up
    mona_comm_gather(comm, send_buf, options->msg_size, recv_buf, 0, 0);
    // benchmark
    MPI_Barrier(MPI_COMM_WORLD);
    t_start = MPI_Wtime();
    ...
    mona_finalize(mona);
    if(rank == 0) printf("MoNA: %d gather of %d bytes executed in %lf sec\n",
            options->iterations, options->msg_size, (t_end-t_start));
}
